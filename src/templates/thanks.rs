...
pub(crate) fn thanks(params: Value, l: Locale) -> Result<Mjml, TemplateError> {
    let ctx: Option<Thanks> = serde_json::from_value(params)?;
    let Thanks {
    ...
    Ok(view! {
        ...
        <mj-head>
            { head().into() }
            <mj-title>{ tl!(l, thanks.title, from_name = from_name_raw).borrow() }</mj-title>
        </mj-head>
        <mj-body width="660px" padding="0">
            ...
            <mj-column padding="0">
                ...
                <mj-text>
                    ...
                    <p>{ Text::from(tl!(l, thanks.info, from_name = from_name)).into() }</p>
                </mj-text>
                    ...
                    <mj-section>
                        <mj-column>
                            ...
                            <mj-button
                                ...
                                padding="10px 0"
                            >
                            { Text::from(tl!(l, thanks.button_text)).into() }
                            </mj-button>
                        </mj-column>
                    </mj-section>
                </mj-wrapper>
